...
from common import (
    DataModel, experiments_by_line, normalize_algo,
)
...
@register_plot
class RelGapLines(Plot):
    @staticmethod
    def key() -> str:
        return "rel-gap-lines"


@staticmethod
def display_name() -> str:
    return "Relative gap (lines)"
...
def run(self, dm: DataModel, out_dir: str) -> None:
    ...
    for line_idx, exps in exp_by_line.items():
        ...
        for exp in exps:
            ...
            for s in exp.steps:
                if s.iteration < 1 or s.iteration > last_iter:
                    continue
                if s.relative_gap is None:
                    continue
                # keep last value per iteration if duplicates
                algo_to_pts_map.setdefault(
                    algo, {})[s.iteration] = float(s.relative_gap)
        ...
        plt.title(f"Relative gap by iteration (line #{line_idx})")
