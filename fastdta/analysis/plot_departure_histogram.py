...
def main():
    ...
    if not os.path.exists(args.trips_file):
        print(
            f"Error: Trips file '{args.trips_file}' not found", file=sys.stderr)
        sys.exit(1)

    if args.n <= 0:
        print(
            f"Error: Time bin size must be positive, got {args.n}", file=sys.stderr)
        sys.exit(1)
    ...
    if departure_times:
        print(
            f"Time range: {min(departure_times):.2f}s to {max(departure_times):.2f}s")
