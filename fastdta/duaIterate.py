...
def addGenericOptions(argParser):
    ...
    argParser.add_argument("-R", "--route-steps", type=int,
                           default=200, help="Set simulation route steps")
...
def initOptions():
    ...
    argParser.add_argument("--max-alternatives", default=5,
                           help="prune the number of alternatives to INT")
    ...
    argParser.add_argument("--logit", action="store_true",
    ...
    argParser.add_argument("-i", "--logitgamma", type=float,
                           default=1., help="use the c-logit model for route choice")
    argParser.add_argument("-G", "--logittheta", type=float,
                           help="parameter to adapt the cost unit")
    argParser.add_argument("-J", "--addweights",
                           help="Additional weights for duarouter")
    ...
    argParser.add_argument("--dualog", default="dua.log",
                           category="output", help="log file path (default 'dua.log')")
                ...
                if "<tripinfo " in line:
                    ...
                    for a in attrs:
                        ...
                        if pos >= 0:
                            pos += len(a) + 2
                            print('%s="%s"' % (
                                a, line[pos:line.find('"', pos)]), end=' ', file=out)
                    hadOutput = True
                if "<emission" in line:
                    for a in attrs:
                        pos = line.find(a)
        ...
        for interval_prv in tree_sumo_prv.getroot():
            ...
            for edge_prv in interval_prv.iter('edge'):
                if edge_prv.get("traveltime") is not None:
                    veh_prv = float(edge_prv.get("left")) + \
                        float(edge_prv.get("arrived"))
                    tt_prv = float(edge_prv.get("overlapTraveltime"))
                    mc_prv = float(edge_prv.get("traveltime"))
                    oldValues[begin_prv][edge_prv.get("id")] = (
                        veh_prv, tt_prv, mc_prv)
...
def main(args=None):
    ...
    for step in range(options.firstStep, options.lastStep):
        ...
        if options.weightmemory:
            ...
            costmemory.load_costs(get_dumpfilename(
                options, step, "dump"), step, get_scale(options, step))
